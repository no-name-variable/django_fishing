...
class FishAI:
    ...
    def calculate_escape_chance(self, tension: float, stamina: float) -> float:
        ...
        - Усталости (слабо держится на крючке)
        - Критическом натяжении (рывок может выбить крючок)
        """
        # Низкий шанс схода когда рыба активная и натяжение умеренное
        if stamina > 60 and tension < 20:
            return 0.001  # Почти нет шанса

        # Повышенный шанс когда рыба устала ИЛИ критическое натяжение
        if stamina < 30 or tension > 70:
            return 0.02  # 2% шанс схода

        return 0.005  # Базовый низкий шанс
